%\VignetteIndexEntry{Sample Guitar2 workflow}
%\VignetteKeywords{Guitar2,visualization,sequencing,RNA methylation,m6A, m5C}
%\VignettePackage{Guitar2}
...
Alternatively, you may also optionally include the promoter DNA region and its complementary region on the 3' side of a transcript in the plot with parameter \Rcode{includeNeighborDNA =TRUE}. 
...
Here is one more example for analysis of DNA m5C methylation using Bismark software from BS-Seq data.

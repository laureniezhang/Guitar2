...
.countDensityCI <- function(ct1, adjust, CI_interval){
  ...
  for (l in 1:length(levels(ct1$Feature))){
    ...
    xx <- ct_tmp$pos
    ww <- ct_tmp$weight/sum(ct_tmp$weight)
    fit1 <- suppressWarnings(density(xx, adjust = adjust, weights = ww)) 

    fit2 <- replicate(10000, { sid <- 1:length(xx);idd <- sample(sid, replace=TRUE);x <- xx[idd];wwx <- ww[idd]; 
    suppressWarnings(density(x, adjust = adjust, weights = wwx)$y)}) # , from=min(fit1$x), to=max(fit1$x)
  ...
  if (includeNeighborDNA) {
    if (rescaleComponent==FALSE) {
      ...
      p1 <- 
        ...
        scale_x_continuous(minor_breaks = seq(0, 7, 1)) + 
      ...
      # # density
      # pro_id <- which(ct1$comp=="Front")
      # tail_id <- which(ct1$comp=="Back")
      # ct1$count[pro_id] <- ct1$count[pro_id]*w1["Front"]
      # ct1$count[tail_id] <- ct1$count[tail_id]*w1["Back"]
      # 
      # cds_id <- which(ct1$comp=="CDS")
      # utr3_id <- which(ct1$comp=="UTR3")
      # utr5_id <- which(ct1$comp=="UTR5")
      # ct1$count[utr5_id] <- ct1$count[utr5_id]*w1["5'UTR"]
      # ct1$count[cds_id] <- ct1$count[cds_id]*w1["CDS"]
      # ct1$count[utr3_id] <- ct1$count[utr3_id]*w1["3'UTR"]
      ...
      # # stratch
      # x <- cumsum(w1)
      # # ct1$pos[utr5_id] <- ct1$pos[utr5_id]*w1["5'UTR"] + 0
      # # ct1$pos[cds_id] <- ct1$pos[cds_id]*w1["CDS"] + x[1]
      # # ct1$pos[utr3_id] <- ct1$pos[utr3_id]*w1["3'UTR"] + x[2]
      # ct1$pos[pro_id] <- ct1$pos[utr5_id]*w1["5'UTR"] + 0
      # ct1$pos[utr5_id] <- ct1$pos[utr5_id]*w1["5'UTR"] + x[1]
      # ct1$pos[cds_id] <- ct1$pos[cds_id]*w1["CDS"] + x[2]
      # ct1$pos[utr3_id] <- ct1$pos[utr3_id]*w1["3'UTR"] + x[3]
      # ct1$pos[tail_id] <- ct1$pos[utr5_id]*w1["5'UTR"] + x[4]
